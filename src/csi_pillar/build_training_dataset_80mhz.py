"""
build_training_dataset_80mhz.py

Same file-level split methodology as build_training_dataset.py, but for
the 256-subcarrier (80 MHz) processed files. Produces a separate HDF5
dataset from the 20 MHz one, since these will train a separate model.

Usage:
    python build_training_dataset_80mhz.py --source "data/mc3_processed_80mhz" --out "data/training_dataset_80mhz.h5"
"""
...
def process_one_file(filepath):
    ...
    if amplitude.shape[1] != 256:
        return None  # not an 80 MHz file
...
def main():
    ...
    print(f"Found {len(all_files)} processed 80MHz files in {source_root}")
    ...
    print(f"\nSaved to {out_path}")
    if skipped:
        print(f"\n{len(skipped)} files skipped due to errors:")
