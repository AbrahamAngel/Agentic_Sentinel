"""
build_training_dataset.py (file-level split version)

Same as before, but this version splits your 216 usable files into
train/validation/test groups FIRST (65% / 17.5% / 17.5%), before ever
building blocks -- so all blocks from a given recording stay together
in the same split. This avoids data leakage: if blocks from the same
file ended up in both train and test, accuracy numbers would look
artificially high and wouldn't reflect real-world performance.

Only processes files with 64 subcarriers (20 MHz recordings) -- the
80 MHz (256-subcarrier) files are skipped for now, same as before.

Usage:
    python build_training_dataset.py --source "data/mc3_processed" --out "data/training_dataset.h5"
"""
...
def process_one_file(filepath):
    """Loads and processes one file, returning None if it's not 64-subcarrier."""
    data = np.load(filepath, allow_pickle=True)
    amplitude = data["amplitude"]
    phase = data["phase"]

    if amplitude.shape[1] != 64:
        return None  # skip 80 MHz / 256-subcarrier files for now
...
def main():
    ...
    # --- Step 1: split FILES (not blocks) into train/val/test ---
    ...
    # --- Step 2: figure out subcarrier count from the first valid 64-subcarrier file ---
    num_subcarriers = 64  # we know this from the earlier run's results
    ...
    with h5py.File(out_path, "w") as hf:
        splits = {
            "train": (train_files, create_split_datasets(hf, "train", num_subcarriers, BLOCK_SIZE), 0),
            "val": (val_files, create_split_datasets(hf, "val", num_subcarriers, BLOCK_SIZE), 0),
            "test": (test_files, create_split_datasets(hf, "test", num_subcarriers, BLOCK_SIZE), 0),
        }

        for split_name, (files, datasets, _) in splits.items():
            current_len = 0
        ...
        # Print label distributions per split for a final sanity check
